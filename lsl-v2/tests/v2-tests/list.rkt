...
(module+ test
  (chk
   ...
   #:x (run (: x (List Boolean)) (define x (list #t 3 #f)) x)  "expected: (Imm"
   ...
   #:x (run (: x (Tuple Integer Boolean)) (define x (list 3 3)) x)  "expected: (Imm"
   ...
   (run (remove-duplicates (list 1 1 2)))
   '(1 2)
